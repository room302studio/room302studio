<template>
  <main
    ...
    <ContentQuery path="/lab" v-slot="{ data }">
      <div v-if="data && data[0]" class="max-w-4xl mx-auto px-4 md:px-6">
        ...
        <!-- Minimal filter bar with improved styling -->
        <div
          class="mb-10 sticky top-4 z-40 py-4 px-6 backdrop-blur-md bg-white/80 dark:bg-stone-900/80 border border-stone-200/70 dark:border-stone-700/70 rounded-xl shadow-sm">
          <div class="flex items-center justify-between flex-wrap gap-y-3">
            <!-- Type filters as minimal text toggles -->
            <div class="flex space-x-8">
              <button @click="activeType = type === activeType ? '' : type" v-for="type in ['video', 'image', 'note']"
                :key="type" :class="[
                  'text-xs uppercase tracking-wider transition-all font-medium px-2 py-1 rounded-full',
                  activeType === type
            ...
            <div class="flex items-center space-x-5">
              <!-- Active tag as text -->
              <div v-if="activeTag" class="flex items-center">
                <span
                  class="text-xs tracking-wide text-primary-700 dark:text-primary-300 font-medium flex items-center gap-1 bg-primary-50 dark:bg-primary-900/30 px-3 py-1 rounded-full">
                  #{{ activeTag }}
                  <button @click="activeTag = ''"
                    class="text-primary-400 hover:text-primary-600 dark:hover:text-primary-200 ml-1">
                    <UIcon name="i-heroicons-x-mark" class="w-3 h-3" />
                  </button>
                </span>
              </div>

              <!-- Reset button - minimal text only -->
              <button v-if="activeType || activeTag" @click="resetFilters"
                class="text-xs uppercase tracking-wider text-stone-400 dark:text-stone-500 hover:text-stone-600 dark:hover:text-stone-300 transition-colors border border-stone-200 dark:border-stone-700 px-3 py-1 rounded-full hover:bg-stone-50 dark:hover:bg-stone-800">
                Clear
              </button>
            </div>
          </div>
        ...
        <div class="lab-timeline relative border-l border-stone-300/50 dark:border-stone-700/50 ml-4 md:ml-8 pb-20">
          <div v-for="(item, index) in filteredLabItems(processedItems)" :key="item.id"
            ...
            <div class="absolute -left-[5px] h-3 w-3 rounded-full bg-primary-500 mt-7 shadow-lg shadow-primary-500/20">
            </div>
            ...
            <div class="ml-6 group">
              ...
              <div
                ...
                <div v-if="item.type === 'video'"
                  ...
                  <div class="absolute bottom-0 left-0 right-0 py-2 px-4 bg-gradient-to-t from-black/70 to-transparent">
              ...
              <div class="cursor-pointer" @click="toggleItemExpansion(item.id)"
                ...
                <h3
                  ...
                  {{ item.title }}
                  <UIcon :name="expandedItems.includes(item.id) ? 'i-heroicons-chevron-up' : 'i-heroicons-chevron-down'"
              ...
              <div v-if="expandedItems.includes(item.id)"
                ...
                <div v-else-if="item.type === 'note'"
                  ...
                  <div class="font-mono text-sm leading-relaxed text-stone-700 dark:text-stone-300 whitespace-pre-wrap"
