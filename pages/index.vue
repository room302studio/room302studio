<template>
  <div class="index">
    <div class="absolute z-0 left-0 top-0">
      <TresCanvas window-size alpha shadows>
        <TresPerspectiveCamera ref="cam" :position="[cameraPosition.x, cameraPosition.y, cameraPosition.z]" :fov="75"
          :near="0.1" :far="1000" />
        ...
        <!-- make all of the spheres -->
        <!-- <TresGroup>
          <TresMesh v-for="sphere in spheres" :position="sphere.position">
            <TresSphereGeometry :args="[sphere.size, 32, 32]" />
            <TresMeshBasicMaterial color="white" />
          </TresMesh>
        </TresGroup> -->

        <!-- make a huge test sphere -->
        <TresGroup>
        ...
        <!-- add ambient lights to accentuate terrain -->
        <TresAmbientLight :intensity="0.5" :position="[0, 0, 0]" />
        />
        ...
        <div class="relative flex my-32">
          <div class="w-1/2">
            ...
            <div class="intro-text text-sm my-2 pl-4 border-l-2 border-gray-500">
              <p class="max-w-prose text-blance">
...
let sphereAnimations, camAnimation
...
onMounted(() => {
  const numSpheres = 2500; // Number of spheres to generate

  for (let i = 0; i < numSpheres; i++) {
    const multiplier = 50;
    const x = noise3D(Math.random(), Math.random(), i) * multiplier;
    const y = noise3D(Math.random(), Math.random(), i) * multiplier;
    const z = noise3D(Math.random(), Math.random(), i) * multiplier;

    const size = 0.01;


    spheres.value.push({ x, y, z, size });
  }

  sphereAnimations = createAnimatable(spheres.value, {
    x: stagger(300),
    y: stagger(300),
    z: stagger(1000),
    ease: 'out(3)'
...
onLoop(({ delta, elapsed }) => {
  if (donut.value) {
    // rotate the donut as the user scrolls
    donut.value.rotation.y = y.value / 1000
  }

  camAnimation.y.to(-y.value / 100)
  // camAnimation.x.to(y.value / 1000)
  camAnimation.z.to(y.value / 2000)
})

// when the zeroToSixty element is visible, we want to gather all the spheres to the center
watch(zeroToSixtyVisible, (visible) => {
  if (visible) {
    console.log('zeroToSixty is visible')
    gatherAllSpheresToCenter()
  }
})

function gatherAllSpheresToCenter() {
  // const newSpherePositions = spheres.value.map((sphere) => {
  //   return { ...sphere, position: [0, 0, 0] }
  // })


  // spheres.value = newSpherePositions
  // sphereAnimations.x.to(0)
  // sphereAnimations.y.to(0)
  // sphereAnimations.z.to(0)
